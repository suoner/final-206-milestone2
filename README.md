...
| :--- | :--- |
| `initial_cohort_query.sql` | **(Milestone 1 Context)** SQL script used to generate the initial, full cohort of all 5,209 patients with a Vitiligo diagnosis from the structured OMOP database. |
| `final_analysis.sql` | **(Primary Milestone 2 Artifact)** A single, comprehensive SQL script that performs the complete analysis for this milestone. It executes the patient sampling, performs pattern matching on the notes, and generates the final pivoted demographic comparison table. |
